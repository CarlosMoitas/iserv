/*
  iServ · Template de Landing Page — lógica de interação
  ------------------------------------------------------------
  Duplique esta pasta para cada nova empresa cliente do iServ e
  ajuste apenas as constantes abaixo com os dados da empresa.
*/
...
(function () {
  ...
  // ---------------------------------------------------------------------
  // 1) Configuração da empresa — AJUSTE AQUI
  // ---------------------------------------------------------------------
  const PRODUCTION_API_URL = "https://iserv-api.onrender.com";
  const PRODUCTION_PAINEL_URL = "https://iserv-l1ji.onrender.com";
  const EMPRESA_SLUG = "minha-empresa"; // slug cadastrado no iServ
  const WHATSAPP_NUMBER = "5511999999999"; // apenas números, com DDI+DDD
  const WHATSAPP_MESSAGE = "Olá! Gostaria de mais informações.";
  const AI_ASSISTANT_NAME = "Assistente"; // nome exibido no widget de chat
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const assistantNameEls = document.querySelectorAll("[data-assistant-name]");
    assistantNameEls.forEach((el) => (el.textContent = AI_ASSISTANT_NAME));
  });

  // ---------------------------------------------------------------------
  // 3) Formulário de contato / lead -> POST /public/:slug/leads
  // ---------------------------------------------------------------------
  document.addEventListener("DOMContentLoaded", function () {
  ...
  //
  // Fluxo alvo do iServ: Cliente clica em "Conversar" -> IA entende
  // necessidade -> consulta disponibilidade -> agenda horário -> registra
  // tudo no iServ -> painel administrativo é atualizado automaticamente.
  //
  // Nesta etapa o widget simula a conversa e já captura o lead usando a
  // rota pública de leads, refletindo no painel administrativo.
  // ---------------------------------------------------------------------
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function startConversation() {
      ...
      started = true;
      addMessage(
        "Olá! 👋 Eu sou o assistente virtual de " + AI_ASSISTANT_NAME + ". Posso te ajudar a entender nossos serviços ou agendar um atendimento. O que você precisa hoje?",
        "bot",
      );
    }
    ...
    function handleUserMessage(text) {
      ...
      window.setTimeout(function () {
        if (step === 0) {
          addMessage(
            "Entendido! Pode me informar seu nome e um telefone ou e-mail? Assim nossa equipe já entra em contato para confirmar o melhor horário.",
            "bot",
          );
          step = 1;
        } else if (step === 1) {
